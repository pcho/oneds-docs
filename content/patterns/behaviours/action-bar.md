...
The action bar is your bulk operations powerhouse—a contextual toolbar that appears when users select items, making it easy to act on many items at once. It shows up across tables, lists, and card grids.

## Core Concept

Action bars are temporary helpers that:
- Only appear when items are selected
- Handle bulk actions on selected items
- Show selection count and status
- Give quick access to common multi-item operations
- Vanish when selection clears or the action completes
