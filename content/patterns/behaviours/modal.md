...
Modals (also called dialogs) are focused overlays that command attention—appearing when you need to display critical information, collect input, or confirm actions. They're interruptions with a purpose.

## Core Concept

Modals are focused helpers that:
- Float on top of all content
- Block interaction with everything else
- Direct user attention to one task
- Need explicit dismissal to continue
- Keep the underlying page context intact
