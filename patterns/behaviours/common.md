...
Common actions are fundamental user interactions that appear consistently throughout the system. They allow users to create, remove, or navigate content efficiently while maintaining a clear and predictable experience. Standardizing these actions ensures usability, consistency, and accessibility across different flows.

If any specific flows need exceptions, document them on the relevant Confluence page and update the corresponding Figma file accordingly.

## 1. Save

### 1.1 Default Standard Behavior

In standard situations, the button must always remain active, regardless of empty fields or unfilled required fields. This ensures that users can proceed and receive error messages upon clicking Save, even if they haven't interacted with the fields.

**Why keep Save active:**
- Disabled buttons don't explain what's wrong
- Avoids prematurely displaying errors
- Prevents slowdown of interaction
...
Only when needed for tooltips or accessibility:
