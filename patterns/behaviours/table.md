...
Tables in our system help users quickly scan records, find what they need, compare data, and take action on selected items. This guide outlines best practices and behaviors for designing effective, user-friendly tables.

## Design Principles

### 1. Set a Clear Goal for Your Table

Most tables are designed for quick scanning to access detailed content. Show only the key information that sets items apart—enough to help users identify and select the right record.

**Good approach:**
- Display essential identifying information (name, ID, status, date)
- Keep columns focused on comparison and selection
- Link to detailed views for comprehensive information

**When users need deep comparison:**
- Let users download a report with all fields
- Provide an "Add columns" feature to customize view
...
### Semantic Structure
- Use `<table>`, `<thead>`, `<tbody>` elements
- Use `<th>` for headers with `scope="col"`
