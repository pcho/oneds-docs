...
Filtering helps users narrow down large datasets to find exactly what they need. Instead of scrolling through hundreds of records, users can apply criteria to show only relevant items—transforming data overload into focused, actionable insights.

Filtering appears consistently across tables, lists, and data views throughout the system.
...
- With active filters: Primary blue (#155EEF) background
...
- Title: "Filters" (18px, font weight 600)
- Close button: X icon (right-aligned)

Content Area:
- Padding: 24px
- Overflow: Scroll vertically
- Gap between filter groups: 24px
...
<aside
  ...
  <!-- Filter Group -->
  <fieldset aria-labelledby="status-label">
    <legend id="status-label">Status</legend>
    <input type="checkbox" id="active" />
    <label for="active">Active (145)</label>
    <!-- More checkboxes -->
  </fieldset>

  <!-- Apply Button -->
  <button aria-controls="results-table">
    Apply Filters
  </button>
</aside>
...
<div role="region" aria-live="polite" aria-atomic="false">
  <p id="results-count">Showing 1,234 of 5,678 patents</p>
</div>
```
...
```
On Filter Applied:
"Filter applied. Showing 1,234 of 5,678 results."

On Filter Removed:
"Status filter removed. Showing 3,456 of 5,678 results."

On Clear All:
"All filters cleared. Showing 5,678 results."
