...
Notifications (toasts) are temporary messages that appear in the interface to provide feedback on actions, system status, or important updates. This guide covers notification behavior patterns for consistent, non-intrusive user feedback.

## Core Concept

Notifications are ephemeral UI elements that:
- Appear automatically after user actions or system events
- Provide quick, non-blocking feedback
- Disappear automatically after a duration
- Stack when multiple appear
- Don't require user interaction (optional dismissal)
...
**Announcements:**
- Screen reader announces notification when it appears
- Reads type: "Success", "Error", "Warning", "Info"
- Reads title and description
- Does not interrupt current task (polite)
- For errors, use assertive to interrupt
