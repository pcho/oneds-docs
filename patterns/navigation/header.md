...
The header is your system's command center—present on every screen, providing consistent access to navigation, search, notifications, and account management. It's the persistent companion that helps users know where they are and get where they need to go, no matter where they start.
...
- Background: White (#FFFFFF) or dark theme
...
- Padding: 0 16px (each item)
- Gap: 4px between items
- Font size: 14px
- Font weight: 500 (medium)
- Color: Gray (#535D67)
- Hover: Primary blue (#155EEF)
- Active: Primary blue (#155EEF) + bottom border

Active Indicator:
- Border bottom: 3px solid blue (#155EEF)
...
States:
Default: Gray text
Hover: Blue text (#155EEF)
Active: Blue text + bottom border
Disabled: Light gray (#7E8A96), no interaction
```
...
<header role="banner">
  ...
  <!-- Logo -->
  <a href="/" aria-label="OneDS Home">
    <img src="logo.svg" alt="OneDS" />
  </a>
    ...
    <ul role="list">
      ...
      <li>
        <button
          aria-haspopup="true"
          aria-expanded="false"
          aria-controls="patents-menu">
          Patents
        </button>
        <ul id="patents-menu" role="menu">
          <li role="menuitem">
  ...
  <!-- Search -->
  <div role="search">
    <button aria-label="Search" aria-keyshortcuts="Meta+K">
...
Dropdown Open:
"Patents menu, expanded, 4 items"

Notification:
"Notifications, 5 unread"

New Notification:
"New notification: Patent application approved" (live region)
```
...
```
Dropdown Open:
1. Focus moves to first item in dropdown
2. Tab cycles within dropdown
3. Escape closes and returns focus to trigger
4. Click outside closes and returns focus to trigger
```
